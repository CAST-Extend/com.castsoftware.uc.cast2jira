...
// SimpleBuildStep
{
    ...
    public static final class DescriptorImpl extends BuildStepDescriptor<Builder> {
        ...
        private JiraHelper getJiraClient(String jiraRestApiUrl, String jiraUser,
            ...
            if (jiraClient == null) {
                if (!jiraRestApiUrl.isEmpty() && !jiraUser.isEmpty()
                        && !jiraUserPassword.isEmpty()) {
                    jiraClient = new JiraHelper(jiraRestApiUrl, jiraUser, jiraUserPassword);
                } else {
                    throw new IllegalArgumentException(
                            "Jira Connection error, not enoug parameters");
                }
            }
            return jiraClient;
        }
        ...
        public FormValidation doTestJiraConnection(
            ...
            try {
                ...
                jiraClient = getJiraClient(jiraRestApiUrl, jiraUser, jiraUserPassword);

                projects = jiraClient.getProjectClient().getAllProjects(pm);
        ...
        public ListBoxModel doFillJiraIssueTypeItems(
            ...
            try {
                jiraClient = getJiraClient(jiraRestApiUrl, jiraUser, jiraUserPassword);
                issueTypes = jiraClient.getMetadataClient().getIssueTypes(pm);
                ...
                try {
                    ...
                    for (BasicProject bp : projects) {
                        if (bp.getKey().equalsIgnoreCase(projName)) {
                            Iterable<BasicComponent> components = jiraClient.getProjectClient()
                                    .getProject(bp.getKey(), pm).getComponents();
