...
public class ActionPlanViolation {
	 ...
	 *            the totales
	 */
	public ActionPlanViolation(
			long object_id, 
			int priority, 
			String actionDate,
			String firstSnapshotDate,
			String actionDef, 
			String objectFullName,
			int metricId,	String metricShortDescription, String reason,
			String metricLongDescription, String remediation, String reference,
			String violationExample, String remediationExample, String output,
			String totales, String sourcePath, int lineStart, int lineEnd, String sourceCode,
			String techCriteria, String businessCriteria, int violationStatus) {
	...
	private String extractSourceCode()
	{
		...
		{
			cLine++;
			if (cLine < srcStartLine) continue;
			else if (cLine > srcEndLine) break;
