...
public class RestClient
{
	...
	private JSON request(HttpRequestBase req) throws RestException, IOException
	{
		...
		// check for a 302 error
		StatusLine sl = resp.getStatusLine();
		if (sl.getStatusCode() == 302) // we are being redirected to a new location
		{
			log.info("HTTP Forward Request error (302) encountered" );
			URI newRequestUri;
			try {
				String newLoc = resp.getFirstHeader("Location").getValue();
				newRequestUri = new URI(newLoc);
				log.info(String.format("New location found:  %s", newLoc));
			} catch (URISyntaxException e) {
				throw new RestException(sl.getReasonPhrase(), sl.getStatusCode(), result.toString());
			}
			req.setURI(newRequestUri);
			return request(req);
		}
	...
	private JSON request(HttpEntityEnclosingRequestBase req, JSON payload) throws RestException, IOException
	{
		...
		if (payload != null) {
			newPayload = payload.toString().replace("\\\"", "\"").replace(":\"{", ":{").replace("}\",", "},")
					.replace("}\"}}", "}}}");
		}

		return request(req, payload != null ? newPayload : null);
